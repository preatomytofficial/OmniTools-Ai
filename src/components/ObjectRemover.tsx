...
import { Scissors, Upload, Download, Sparkles, Move, Film, Image as ImageIcon, CheckCircle2 } from 'lucide-react';
...
export const ObjectRemover: React.FC = () => {
  ...
  const processSelectedFile = (file: File) => {
    ...
    if (!isVid) {
      setBox({ x: 60, y: 60, w: 140, h: 140 });
    } else {
      setBox({ x: 40, y: 40, w: 160, h: 120 });
    }
  };

  const loadSampleImage = async () => {
    try {
      const res = await fetch('/samples/sample_object.png');
      const blob = await res.blob();
      const file = new File([blob], 'sample_object.png', { type: 'image/png' });
      processSelectedFile(file);
    } catch {
  ...
  const drawSelectionBox = (ctx: CanvasRenderingContext2D) => {
    ...
    ctx.fillText('ERASE OBJECT REGION', box.x + 4, Math.max(14, box.y - 6));
  };
  ...
  const handleProcessObject = async () => {
    if (!selectedFile) {
    ...
    try {
      const data = await safeApiFetch<{ success?: boolean; isVideo?: boolean; base64Image?: string; outputUrl?: string; error?: string }>('/api/remove-object', {
        method: 'POST',
        body: formData,
      });
      ...
      setError(err.message || 'Error executing object remover.');
    } finally {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-white tracking-tight">Object Remover & Eraser (Images & Videos)</h2>
          </div>
          <p className="text-sm text-gray-400 mt-1">
            Seamless boundary-aware context fill to eliminate unwanted objects, photobombers, wires, and distractions.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="text-xs text-rose-400 hover:text-rose-300 font-medium px-3 py-1.5 rounded-lg bg-rose-950/40 border border-rose-800/40 hover:bg-rose-900/40 transition-colors flex items-center gap-1.5"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-5">
          <div
            ...
            className="border-2 border-dashed border-white/15 hover:border-rose-500/50 bg-black/20 hover:bg-rose-950/10 rounded-2xl p-6 text-center cursor-pointer transition-all group"
          >
            <input
              ref={fileInputRef}
              type="file"
              ...
              className="hidden"
            />
            <div className="w-12 h-12 rounded-xl bg-rose-500/10 text-rose-400 flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform">
            ...
            {selectedFile && (
              <span className={`inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 mt-2 rounded-full border ${
                isVideo ? 'bg-indigo-500/10 text-indigo-300 border-indigo-500/30' : 'bg-rose-500/10 text-rose-300 border-rose-500/30'
              }`}>
            ...
            {isVideo && videoDuration > 0 && (
              <div className="pt-2 border-t border-white/10 space-y-1.5">
                <div className="flex items-center justify-between text-xs text-gray-300 font-medium">
                  ...
                  <span className="font-mono text-rose-300">{currentTime.toFixed(1)}s / {videoDuration.toFixed(1)}s</span>
                </div>
                <input
                  type="range"
                  min={0}
                  ...
                  className="w-full accent-rose-400 bg-white/10 rounded-lg h-2 cursor-pointer"
                />
                <p className="text-[11px] text-gray-400">Scrub timeline to locate and box the object in the video frame.</p>
              </div>
            )}

            <button
              onClick={handleProcessObject}
              ...
              className={`w-full py-3 px-4 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${
                !selectedFile || isProcessing
                  ...
                  : 'bg-gradient-to-r from-rose-600 to-indigo-600 hover:from-rose-500 hover:to-indigo-500 text-white shadow-lg shadow-rose-500/25'
              }`}
            >
              {isProcessing ? (
                <>
                  <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  <span>{isVideo ? 'Erasing Object Across Frames...' : 'Erasing Unwanted Object...'}</span>
                </>
              ) : (
                <>
                  <Sparkles className="w-4 h-4" />
                  <span>{isVideo ? 'Erase Object from Video' : 'Erase Object from Image'}</span>
                </>
              )}
            </button>
          ...
          <div className="bg-[#121826]/80 border border-white/10 rounded-2xl p-4 flex flex-col items-center justify-center min-h-[440px]">
            {mediaSrc ? (
              <div className="w-full flex flex-col items-center">
                <div className="w-full flex items-center justify-between mb-2 text-xs text-gray-300">
                  <span className="flex items-center gap-1.5">
                    ...
                    Drag cursor/touch over canvas to mark object area:
                  </span>
                  <span className="font-mono text-rose-400 font-bold">{box.w}x{box.h} px</span>
                </div>
                  ...
                  <div className="w-full mt-5 bg-black/50 p-4 rounded-xl border border-emerald-500/30 text-center space-y-3 shadow-xl">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <span className="text-xs font-bold text-emerald-400 flex items-center gap-1.5">
                        ...
                        {resultIsVideo ? 'Object-Free Video Output (.mp4):' : 'Cleaned Image Output:'}
                      </span>
                      <a
                        href={resultUrl}
                        download={resultIsVideo ? 'object_erased.mp4' : 'object_erased.png'}
              ...
              <div className="text-center p-8 text-gray-500 space-y-3">
                <Scissors className="w-12 h-12 mx-auto stroke-1 text-gray-600" />
                <p className="text-sm font-semibold text-gray-300">No Image or Video Loaded</p>
                <p className="text-xs text-gray-400 max-w-sm mx-auto">
                  Upload an image or video to erase unwanted objects, or click one of the demo buttons above to test inpainting.
                </p>
              </div>
            )}
          </div>
